...
class biModel extends model
{
     ...
     */
    public function getFields($statement)
    {
     ...
     * @param object $statement
     */
    public function getTables($statement, $deep = false)
    {
        $tables = array();
        if($statement->from)
        {
    ...
    public function getFieldsWithAlias($sql)
    {
        ...
        foreach($statement->expr as $expr)
        {
            ...
            if(substr_compare($expr->expr, '*', -strlen('*')) === 0)
            {
                $fields = $this->dev->getFields($table);
     ...
     * @return array|false
     */
    public function getColumns($sql, $driver = 'mysql', $returnOrigin = false)
    {
     ...
     * @return object
     */
    public function getColumnsType($sql, $driverName = 'mysql', $columns = array())
    {
     ...
     * @return array
     */
    public function getOptionsFromSql($sql, $driver, $keyField, $valueField)
    {
        $options = array();
        $dbh     = $this->app->loadDriver($driver);
     ...
     * @return object
     */
    public function buildSQL($selects, $from, $joins = array(), $functions = array(), $wheres = array(), $querys = array(), $groups = array())
    {
        $this->app->loadClass('sqlparser', true);
        $parser = new sqlparser(null);
     ...
     * @return object
     */
    public function rebuildFieldSettings($fieldPairs, $columns, $relatedObject, $fieldSettings, $objectFields)
    {
     ...
     * @param string $status
     */
    public function convertDataForDtable($data, $configs, $version, $status)
    {
        $columns      = array();
        $rows         = array();
        $cellSpan     = array();
     ...
     * @return array
     */
    public function getDrillFields($rowIndex, $columnKey, $drills)
    {
     ...
     * @return array
     * @param string $field
     * @param mixed[] $drillFields
     * @param mixed[] $columns
     */
    public function processDrills($field, $drillFields, $columns)
    {
     ...
     * @return array
     */
    public function prepareDrillConditions($drillFields, $conditions, $originField)
    {
     ...
     * @return string
     */
    public function downloadDuckdb()
    {
     ...
     * @return string
     */
    public function updateDownloadingTagFile($type = 'file', $action = 'create')
    {
     ...
     * @return bool
     */
    public function unzipFile($path, $file, $extractFile)
    {
     ...
     * @return bool
     */
    public function downloadFile($url, $savePath, $finalFile)
    {
        $ch = curl_init($url);
