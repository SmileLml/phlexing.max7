...
formBatchPanel(h::input
(
    set::type('hidden'),
    set::name('serverType'),
    set::value(ucfirst($server->type))
), set::id('repoList'), set::back('repo-maintain'), set::mode(count($repoList) == 0 ? 'edit' : 'add'), set::addRowIcon('false'), set::items($items), set::data($repoList), set::maxRows(count($repoList)));
