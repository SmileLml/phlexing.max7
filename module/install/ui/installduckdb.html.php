...
{
    return p(setClass("$type-$stus" . ($show ? '' : ' hidden')), icon
    (
        setClass($iconClass[$stus]),
        $icons[$stus]
    ), $type == 'duckdb'    ? sprintf($duckdbLang[$stus], $lang->install->duckdb) : null, $type == 'ext_dm' || $type == 'ext_mysql' ? sprintf($extLang[$stus], $type) : null);
};
