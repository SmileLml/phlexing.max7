...
featureBar
(
    $this->config->vision == 'lite' ? dropdown(btn(setClass('dropdown-btn'), $execution->name), set::items($executionItems)) : null,
    (($features['story'] or $features['qa']) && $this->config->vision != 'lite') ? inputControl
    (
        setClass('c-type'),
        picker(set::width('200'), set::name('type'), set::items($lang->kanban->type), set::value($browseType), set::required(true), set::onchange('changeBrowseType()'))
    ) : null,
    $browseType != 'all' && $this->config->vision != 'lite' ? inputControl
    (
        setClass('c-group ml-5'),
        picker(set::width('200'), set::name('group'), set::items($lang->kanban->group->$browseType), set::value($groupBy), set::required(true), set::onchange('changeGroupBy()'))
    ) : null,
...
(
    ...
    (
        btn(setClass('ghost btn square btn-default'), set::icon('ellipsis-v')),
        set::caret(false),
        set::items($operationMenu)
    ) : null,
    $createMenu ? dropdown
    (
        btn(setClass('primary btn btn-default create-btn'), set::icon('plus'), $lang->create),
