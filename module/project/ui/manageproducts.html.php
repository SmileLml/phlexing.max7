...
foreach($linkedProducts as $productID => $product)
{
    ...
    {
        $linkedList[] = btn(setClass('product-block modal-content center-row justify-start text-left'), checkbox
        (
        ...
        (
            ...
            set::disabled(true),
            set::required(true)
        ) : null, $cannotUnlink ? input
        (
            set::type('hidden'),
            set::name("products[{$index}]"),
            set::value($productID)
        ) : null, $cannotUnlink || isset($allBranches[$productID][$branchID]) ? input
        (
            set::type('hidden'),
            set::name("branch[{$index}]"),
            set::value($branchID)
