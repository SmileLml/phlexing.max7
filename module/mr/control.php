...
class mr extends control
{
     ...
     * @return void
     */
    public function browseByExecution($repoID = 0, $mode = 'status', $param = 'opened', $projectID = 0, $orderBy = 'id_desc', $recTotal = 0, $recPerPage = 20, $pageID = 1)
    {
        if($param == 'assignee' || $param == 'creator')
        {
            $mode  = $param;
     ...
     * @return void
     */
    public function create($repoID = 0, $objectID = 0)
    {
        if($_POST)
        {
     ...
     * @return void
     */
    public function link($MRID, $type = 'story', $orderBy = 'id_desc', $param = '', $recPerPage = 20, $pageID = 1)
    {
        $MR       = $this->mr->fetchByID($MRID);
        ...
        $stories = $this->mr->getLinkList($MRID, 'story', $type == 'story' ? $orderBy : '', $storyPager);
        $bugs    = $this->mr->getLinkList($MRID, 'bug',   $type == 'bug'   ? $orderBy : '', $bugPager);
        $tasks   = $this->mr->getLinkList($MRID, 'task',  $type == 'task'  ? $orderBy : '', $taskPager);
     ...
     * @return void
     */
    public function linkStory($MRID, $repoID = 0, $browseType = '', $param = 0, $orderBy = 'id_desc', $recTotal = 0, $recPerPage = 10, $pageID = 1)
    {
        if(!empty($_POST['stories']))
        {
            $this->mr->link($MRID, 'story', $this->post->stories);
            if(dao::isError()) return $this->sendError(dao::getError());
     ...
     * @return void
     */
    public function linkBug($MRID, $repoID = 0, $browseType = '', $param = 0, $orderBy = 'id_desc', $recTotal = 0, $recPerPage = 10, $pageID = 1)
    {
        if(!empty($_POST['bugs']))
        {
            $this->mr->link($MRID, 'bug', $this->post->bugs);
        ...
        $this->loadModel('bug');
        $queryID   = ($browseType == 'bysearch') ? (int)$param : 0;
        $this->mrZen->buildLinkBugSearchForm($MRID, $repoID, $orderBy, $queryID);
     ...
     * @return void
     */
    public function linkTask($MRID, $repoID = 0, $browseType = 'unclosed', $param = 0, $orderBy = 'id_desc', $recTotal = 0, $recPerPage = 10, $pageID = 1)
    {
        if(!empty($_POST['tasks']))
        {
            $this->mr->link($MRID, 'task', $this->post->tasks);
            if(dao::isError()) return $this->sendError(dao::getError());
        ...
        $linkedTasks = $this->mr->getLinkList($MRID, 'task');
     ...
     * @return void
     */
    public function unlink($MRID, $type, $linkID)
    {
     ...
     * @return void
     */
    public function commitLogs($MRID)
    {
        $MR      = $this->mr->fetchByID($MRID);
        $compile = $this->loadModel('compile')->getById($MR->compileID);
    ...
    * @return void
    */
   public function ajaxGetBranchPrivs($hostID, $project)
   {
    ...
    * @return void
    */
   public function ajaxSyncMRs($repoID)
   {
    ...
    * @return void
    */
   public function ajaxExecJob($MRID, $jobID)
   {
    ...
    * @return void
    */
   public function ajaxLinkObjects($MRID)
   {
       $mr = $this->mr->fetchByID($MRID);
       if(!$mr) return false;
    ...
    * @return void
    */
   public function ajaxSyncCompile($compileID)
   {
