...
$getMergeData = function($data)
{
    ...
    foreach($data->productlines as $line)
    {
        ...
        (
            setClass('flex'),
            cell(set::width('1/3'), setClass('productList px-4 flex items-center border-t mr-2 overflow-hidden'), set::style(array('background-color' => 'var(--color-gray-50)')), checkbox(setID("productLines{$line->id}"), set::name("productLines[$line->id][]"), set::text($line->name), set::value($line->id), setData(array('on' => 'change', 'call' => 'changeLines', 'params' => 'event')))),
            cell
            (
                set::flex('1'),
