...
class instanceModel extends model
{
     ...
     * @return int
     */
    public function getInstanceCount($status = '')
    {
     ...
     * @return array
     */
    public function getList($pager = null, $pinned = '', $searchParam = '', $status = 'all', $alertMark = true)
    {
     ...
     * @return bool
     */
    public function updateMemorySize($instance, $size = 0)
    {
        $settings = new stdclass;
        $settings->settings_map = new stdclass;
     ...
     * @return bool
     */
    public function updateCpuSize($instance, $size = 0)
    {
        $settings = new stdclass;
        $settings->settings_map = new stdclass;
        $settings->settings_map->resources = new stdclass;
     ...
     * @return bool
     */
    public function updateVolSize($instance, $size, $name)
    {
     ...
     * @return array
     */
    public static function printStorageUsage($instance, $metrics)
    {
        if($instance->source === 'user') return array('color' => '', 'tip' => '', 'rate' => '', 'usage' => '', 'limit' => '');
        $rate = $instance->status == 'stopped' ? 0 : $metrics->rate;
     ...
     * @return bool
     */
    public function checkAppNameUnique($name)
    {
     ...
     * @return bool
     */
    public function saveAuthInfo($instance)
    {
     ...
     * @return string
     */
    public function generatePipelineName($instance)
    {
        $name = $instance->name;
        $type = $instance->chart;
     ...
     * @return bool
     */
    public function cleanBackup($instance, $user)
    {
        $instance->spaceData = $this->dao->select('*')->from(TABLE_SPACE)->where('id')->eq($instance->space)->fetch();
