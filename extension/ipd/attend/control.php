...
class attend extends control
{
    ...
    {
        die($this->fetch('attend', 'browse', "date=$date", ''));
    }
    ...
    {
        die($this->fetch('attend', 'browse', "date=$date&company=true", ''));
    }
    ...
    public function browse($date = '', $company = false)
    {
        ...
        {
            $deptList = array();
            $depts    = $this->loadModel('dept')->getList();
        ...
        $newUsers = array();
        foreach($users as $user) $newUsers[$user->account] = $user;
    ...
    public function settings($module = '')
    {
        if($_POST)
        {
            ...
            $this->loadModel('setting')->setItems('system.attend', $settings);
        ...
        $this->view->users          = $this->user->getDeptPairs('noempty,noclosed,nodeleted,noforbidden');
    ...
    public function sendmail($attendID, $actionID)
    {
        ...
        if($action->action == 'commited')
        {
            ...
            else
            {
               ...
               if(!empty($dept->manager)) $toList = trim($dept->manager, ',');
            }
        }
        $label = '';
    ...
    public function setManager($module = '')
    {
        ...
        $this->view->deptList = $this->loadModel('dept')->getList();
        $this->view->users    = $this->loadModel('user')->getDeptPairs('noclosed,nodeleted,noforbidden');
        $this->view->module   = $module;
        $this->display();
    }
    ...
    {
        $users = $this->loadModel('user')->getDeptPairs('noclosed,nodeleted,noforbidden', $deptID);
        $html  = '';
    ...
    public function exportDetail($date = '')
    {
        ...
        if($_POST)
        {
            ...
            if(!empty($attends))
            {
                foreach($attends as $attend) $attend->status = $attend->desc ? $attend->desc : '';
            }
    ...
    {
        $today = helper::today();
        if(!isset($this->config->attend->readers->$today)) $this->loadModel('setting')->deleteItems("owner=system&module=attend&section=readers");
        ...
        $this->loadModel('setting')->setItem("system.attend.readers.{$today}", $readers);
    }
}
