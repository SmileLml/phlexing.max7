...
class durationestimation extends control
{
    ...
    public function index($projectID)
    {
        ...
        $stages  = $this->loadModel('stage')->getStages('id_asc', $projectID);
        ...
        $this->view->title          = $this->lang->durationestimation->common . $this->lang->hyphen . $project->name;
    ...
    public function create($projectID = 0)
    {
        ...
        if(!empty($_POST))
        {
            ...
            if($total != $workloadTotal) return $this->send(array('result' => 'fail', 'message' => $this->lang->durationestimation->workloadError));

            $this->durationestimation->save($projectID);

            if(dao::isError()) return $this->send(array('result' => 'fail', 'message' => dao::getError()));
            return $this->send(array('result' => 'success', 'message' => $this->lang->saveSuccess, 'locate' => inlink('index', "projectID={$projectID}")));
        }
        ...
        $this->view->position[]     = $this->lang->durationestimation->common;
        $this->view->project        = $project;
        $this->view->stages         = $this->loadModel('stage')->getStages('id_asc', $projectID);
    ...
    public function ajaxGetDuration($projectID, $stage, $workload, $worktimeRate, $people, $startDate)
    {
        ...
        if($startDate == '0000-00-00') return $this->send(array('result' => 'success', 'endDate' => '0000-00-00'));
        ...
        if(!$divisor) return $this->send(array('result' => 'fail'));
        ...
        for($i = 0; $days < $duration; $i ++)
        {
            ...
            if($this->config->execution->weekend == 2)
            {
                if($day > 5) continue;
            }
            if($this->config->execution->weekend == 1)
            {
                if($day > 6) continue;
            }
        ...
        return $this->send(array('result' => 'success', 'endDate' => $lastDay));
    }
}
