...
class measurementModel extends model
{
    ...
    public function getReports($type, $pager = null)
    {
        ...
        if($type == 'pairs') return arrayUnion(array('' => ''), $pairs);
    ...
    {
        return $this->dao->select('program,id')->from(TABLE_PRODUCT)
            ->where('deleted')->eq('0')
    ...
    public function getControlOptions($optionType)
    {
        ...
        if(!isset($optionList[$optionType]))
        {
            ...
            {
                $options = arrayUnion(array('' => ''), $this->loadModel('project')->getPairsByProgram());
            }
            elseif($optionType == 'product')
            {
                $options = arrayUnion(array('' => ''), $this->loadModel('product')->getPairs('nocode'));
            }
            elseif($optionType == 'sprint')
            {
                $options = arrayUnion(array('' => ''), $this->loadModel('execution')->getPairs());
            }
            ...
            elseif(strpos($optionType, '.') !== false)
            {
                ...
                unset($options[0]);
                unset($options['']);
                $options = arrayUnion(array('' => ''), $options);
            }
            else
            {
                $options = array('' => '');
