...
jsVar('from',         $from);
jsVar('isVerify',     isset($isVerify) ? $isVerify : false);

jsVar('isModuleCalcExist', isset($isModuleCalcExist) ? $isModuleCalcExist : false);
jsVar('moduleCalcTip',     isset($moduleCalcTip) ? $moduleCalcTip : '');
...
(
    ...
    (
        ...
        label
        (
            to::before(icon(setClass('warning-ghost margin-left8'), 'help')),
...
(
    ...
    (
        ...
        on::click('.btn-download', "download({$metric->id})")
    ),
    div
    (
        h1
        (
            setClass('border-bottom'),
            span
            (
