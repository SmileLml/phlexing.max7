...
class overtime extends control
{
    ...
    {
        die($this->fetch('overtime', 'browse', "type=personal&date=$date&orderBy=$orderBy", ''));
    }
    ...
    {
        die($this->fetch('overtime', 'browse', "type=browseReview&date=$date&orderBy=$orderBy", ''));
    }
    ...
    {
        die($this->fetch('overtime', 'browse', "type=company&date=$date&orderBy=$orderBy", ''));
    }
    ...
    public function edit($id)
    {
        ...
        {
            $locate     = helper::safe64Encode(helper::createLink('overtime', 'browse'));
    ...
    public function setReviewer($module = '')
    {
        if($_POST)
        {
            $this->loadModel('setting')->setItem('system.overtime..reviewedBy', $this->post->reviewedBy);
            if(dao::isError()) $this->send(array('result' => 'fail', 'message' => dao::getError()));
            $this->send(array('result' => 'success', 'message' => $this->lang->saveSuccess, 'locate' => 'reload'));
        }
