...
class makeup extends control
{
    ...
    {
        die($this->fetch('makeup', 'browse', "type=personal&date=$date&orderBy=$orderBy", ''));
    }
    ...
    {
        die($this->fetch('makeup', 'browse', "type=browseReview&date=$date&orderBy=$orderBy", ''));
    }
    ...
    {
        die($this->fetch('makeup', 'browse', "type=company&date=$date&orderBy=$orderBy", ''));
    }
    ...
    public function create($date = '')
    {
        ...
        $this->view->leaves = $this->loadModel('leave')->getPairs('company', '', '', $this->app->user->account, '', 'pass');
        $this->view->date   = $date;
        $this->display();
    }
    ...
    public function edit($id)
    {
        ...
        {
            $locate     = helper::safe64Encode(helper::createLink('makeup', 'browse'));
    ...
    public function view($id, $type = '')
    {
        ...
        $leaves    = array('');
        $leaveList = $this->loadModel('leave')->getByIdList(trim($makeup->leave, ','));
    ...
    public function switchStatus($makeupID)
    {
        ...
        if($makeup->status == 'wait')
        {
            ...
            $this->loadModel('attend')->batchUpdate($dates, $makeup->createdBy);
        }
        ...
        if($makeup->status == 'draft')
        {
            ...
            $this->loadModel('attend')->batchUpdate($dates, $makeup->createdBy, '', 'makeup');
        }
    ...
    public function sendmail($makeupID, $actionID)
    {
        ...
        if($action->action == 'created' or $action->action == 'revoked' or $action->action == 'commited')
        {
            ...
            else
            {
               ...
               if($dept) $toList = trim($dept->manager, ',');
            }
    ...
    public function setReviewer($module = '')
    {
        if($_POST)
        {
            $this->loadModel('setting')->setItem('system.makeup..reviewedBy', $this->post->reviewedBy);
            if(dao::isError()) $this->send(array('result' => 'fail', 'message' => dao::getError()));
            $this->send(array('result' => 'success', 'message' => $this->lang->saveSuccess, 'locate' => 'reload'));
        }
