...
class leave extends control
{
    ...
    {
        die($this->fetch('leave', 'browse', "type=personal&date=$date&orderBy=$orderBy", ''));
    }
    ...
    {
        die($this->fetch('leave', 'browse', "type=browseReview&date=$date&orderBy=$orderBy", ''));
    }
    ...
    {
        die($this->fetch('leave', 'browse', "type=company&date=$date&orderBy=$orderBy", ''));
    }
    ...
    public function edit($id)
    {
        ...
        if($this->app->user->admin != 'super' && $leave->createdBy != $this->app->user->account)
        {
            ...
            if(!$reviewedBy)
            {
                $createdUser = $this->loadModel('user')->getByAccount($leave->createdBy);
                $dept        = $this->loadModel('dept')->getByID($createdUser->dept);
                $reviewedBy  = empty($dept) ? '' : trim($dept->manager, ',');
            }

            if($this->app->user->account != $reviewedBy)
            {
                $locate     = helper::safe64Encode(helper::createLink('leave', 'browse'));
    ...
    public function back($leaveID = 0)
    {
        ...
        {
            $locate     = helper::safe64Encode(helper::createLink('leave', 'personal'));
            $noticeLink = helper::createLink('notice', 'index', "type=accessLimited&locate={$locate}");
            die(js::locate($noticeLink));
        }
    ...
    public function setReviewer($module = '')
    {
        if($_POST)
        {
            $this->loadModel('setting')->setItem('system.leave..reviewedBy', $this->post->reviewedBy);
            if(dao::isError()) $this->send(array('result' => 'fail', 'message' => dao::getError()));
            $this->send(array('result' => 'success', 'message' => $this->lang->saveSuccess, 'locate' => 'reload'));
        }
    ...
    public function personalAnnual($module = '')
    {
        ...
        $this->view->users  = $this->loadModel('user')->getDeptPairs('noclosed,nodeleted,noforbidden');
        $this->view->module = $module;
        $this->display();
    }
}
