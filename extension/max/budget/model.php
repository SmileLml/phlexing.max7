...
class budgetModel extends model
{
    ...
    public function create()
    {
        ...
        $budget = $this->loadModel('file')->processImgURL($budget, $this->config->budget->editor->create['id'], $this->post->uid);
    ...
    public function update($budgetID)
    {
        ...
        $budget = $this->loadModel('file')->processImgURL($budget, $this->config->budget->editor->edit['id'], $this->post->uid);
        $this->dao->update(TABLE_BUDGET)->data($budget)
            ->where('id')->eq($budgetID)
            ->autoCheck()
            ->batchCheck($this->config->budget->edit->requiredFields, 'notempty')
            ->exec();

        if(dao::isError()) return false;
        return common::createChanges($oldBudget, $budget);
    }
}
