      ...
      <div class='tab-content' id="todoLists">
        ...
        <div class='tab-pane fade <?php if($type == 'task') echo 'active in';?>' id='tab_<?php echo $type;?>'>
          ...
          <?php foreach($todoSides as $id => $todo):?>
            ...
            <?php if($type == 'task') $type = $this->config->vision == 'or' ? 'researchtask' : $type;?>
            <?php $method = $type == 'feedback' ? 'adminView' : 'view';?>
            <li data-index='<?php echo $i++;?>'>
              <?php $class = common::hasPriv('todo', 'create') ? 'todo-item' : 'todoList'?>
              <?php echo html::a($this->createLink($type, $method, "id=$id", 'html', true), $todo, '', "class='iframe $class' data-id='{$id}' data-type='{$type}' data-title='{$todo}' title='{$todo}' data-width='90%'");?>
            </li>
          <?php endforeach;?>
          </ul>
          <?php endif;?>
    ...
    }).on('click', '.todo-check', function(e)
    {
        ...
        $.ajax({
              ...
              success: function(data)
              {
                  ...
                  if(actionUrl == config.ajaxFinishUrl && $.isPlainObject(data['load']))
                  {
                      if(confirm(data['load']['confirm']) == true) window.$.apps.openApp(data['load']['confirmed']['url'], data['load']['confirmed']['app']);
                  }
