...
trait HttpClientTrait
{
     ...
     */
    private static function prepareRequest($method, $url, $options, $defaultOptions = [], $allowExtraOptions = false)
    {
        if (null !== $method) {
     ...
     * @param bool $allowExtraOptions
     */
    private static function mergeDefaultOptions($options, $defaultOptions, $allowExtraOptions = false)
    {
     ...
     * @param mixed[] $normalizedHeaders
     */
    private static function normalizeBody($body, &$normalizedHeaders = [])
    {
        if (\is_array($body)) {
            static $cookie;
            ...
            array_walk_recursive($body, $caster = static function (&$v) use (&$caster, &$streams, &$cookie) {
                if (\is_resource($v) || $v instanceof StreamableInterface) {
                    $cookie = hash('xxh128', $cookie = $cookie ?? random_bytes(8), true);
            ...
            if ('' === $body || !$streams && strpos($normalizedHeaders['content-type'][0] ?? '', 'multipart/form-data') === false) {
                if (strpos($normalizedHeaders['content-type'][0] ?? '', 'application/x-www-form-urlencoded') === false) {
            ...
            if (preg_match('{multipart/form-data; boundary=(?|"([^"\r\n]++)"|([-!#$%&\'*+.^_`|~_A-Za-z0-9]++))}', $normalizedHeaders['content-type'][0] ?? '', $boundary)) {
                ...
                $boundary = substr(strtr(base64_encode($cookie = $cookie ?? random_bytes(8)), '+/', '-_'), 0, -2);
            ...
            foreach ($body as $i => $part) {
                ...
                foreach (\is_array($headers) ? $headers : [] as $h) {
                    if (\is_string($h) && 0 === stripos($h, 'Content-Type: ')) {
                        $contentType = $contentType ?? substr($h, 14);
     ...
     */
    private static function normalizePeerFingerprint($fingerprint)
    {
     ...
     * @param int $maxDepth
     */
    private static function jsonEncode($value, $flags = null, $maxDepth = 512)
    {
     ...
     */
    private static function resolveUrl($url, $base, $queryDefaults = [])
    {
     ...
     */
    private static function parseUrl($url, $query = [], $allowedSchemes = ['http' => 80, 'https' => 443])
    {
     ...
     * @param bool $replace
     */
    private static function mergeQueryString($queryString, $queryArray, $replace)
    {
        if (!$queryArray) {
            return $queryString;
        }
     ...
     */
    private static function getProxy($proxy, $url, $noProxy)
    {
     ...
     * @param mixed[] $url
     */
    private static function getProxyUrl($proxy, $url)
    {
        if (null !== $proxy) {
            return $proxy;
        }
     ...
     * @param mixed[] $headers
     */
    private static function shouldBuffer($headers)
    {
