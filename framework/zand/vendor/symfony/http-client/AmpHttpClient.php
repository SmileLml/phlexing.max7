...
final class AmpHttpClient implements HttpClientInterface, LoggerAwareInterface, ResetInterface
{
     ...
     */
    public function __construct($defaultOptions = [], $clientConfigurator = null, $maxHostConnections = 6, $maxPendingPushes = 50)
    {
        ...
        if ($options['http_version']) {
            switch ((float) $options['http_version']) {
                case 1.0:
                    ...
                    break;
                case 1.1:
                    $request->setProtocolVersions($request->setProtocolVersions(['1.1', '1.0']));
                    break;
                default:
    ...
    public function reset()
    {
        ...
        foreach ($this->multi->pushedResponses as $authority => $pushedResponses) {
            foreach ($pushedResponses as [$pushedUrl, $pushDeferred]) {
                ...
                ($logger = $this->logger) ? $logger->debug(sprintf('Unused pushed response: "%s"', $pushedUrl)) : null;
            }
        }
