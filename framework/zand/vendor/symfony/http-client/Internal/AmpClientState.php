...
final class AmpClientState extends ClientState
{
     ...
     */
    public function __construct($clientConfigurator, $maxHostConnections, $maxPendingPushes, &$logger)
    {
     ...
     * @param \Amp\CancellationToken $cancellation
     * @param mixed[] $info
     * @param \Closure $onProgress
     */
    public function request($options, $request, $cancellation, &$info, $onProgress, &$handle)
    {
        if ($options['proxy']) {
    ...
    private function getClient($options)
    {
        ...
        $connector = $handleConnector = new class() implements Connector {
            ...
            public function connect(string $uri, ConnectContext $context = null, CancellationToken $token = null)
            {
    ...
    /**
     * @param \Amp\Http\Client\Request $request
     * @param \Amp\Promise $response
     * @param mixed[] $options
     */
    private function handlePush($request, $response, $options)
    {
