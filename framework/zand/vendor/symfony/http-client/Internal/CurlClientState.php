...
final class CurlClientState extends ClientState
{
    ...
    /**
     * @var \Symfony\Component\HttpClient\Internal\DnsCache
     */
    public $dnsCache;
    /** @var float[] */
     ...
     */
    public function __construct($maxHostConnections, $maxPendingPushes)
    {
    ...
    public function reset()
    {
        foreach ($this->pushedResponses as $url => $response) {
            ($logger = $this->logger) ? $logger->debug(sprintf('Unused pushed response: "%s"', $url)) : null;
     ...
     * @param int $maxPendingPushes
     */
    private function handlePush($parent, $pushed, $requestHeaders, $maxPendingPushes)
    {
        $headers = [];
        ...
        if (!isset($headers[':method']) || !isset($headers[':scheme']) || !isset($headers[':authority']) || !isset($headers[':path'])) {
            ($logger = $this->logger) ? $logger->debug(sprintf('Rejecting pushed response from "%s": pushed headers are invalid', $origin)) : null;
        ...
        if (strncmp($origin, $url.'/', strlen($url.'/')) !== 0) {
            ($logger = $this->logger) ? $logger->debug(sprintf('Rejecting pushed response from "%s": server is not authoritative for "%s"', $origin, $url)) : null;
        ...
        if ($maxPendingPushes <= \count($this->pushedResponses)) {
            ...
            ($logger = $this->logger) ? $logger->debug(sprintf('Evicting oldest pushed response: "%s"', $fifoUrl)) : null;
        }

        $url .= $headers[':path'][0];
        ($logger = $this->logger) ? $logger->debug(sprintf('Queueing pushed response: "%s"', $url)) : null;
