...
final class AmpResponse implements ResponseInterface, StreamableInterface
{
    ...
    /**
     * @var \Symfony\Component\HttpClient\Internal\AmpClientState
     */
    private $multi;
    /**
     * @var mixed[]|null
    ...
    /**
     * @internal
     * @param \Symfony\Component\HttpClient\Internal\AmpClientState $multi
     * @param \Amp\Http\Client\Request $request
     * @param mixed[] $options
     * @param \Psr\Log\LoggerInterface|null $logger
     */
    public function __construct($multi, $request, $options, $logger)
    {
        $this->multi = $multi;
     ...
     * @param float $timeout
     */
    private static function select($multi, $timeout)
    {
     ...
     * @param mixed[] $info
     * @param mixed[] $headers
     * @param \Amp\CancellationTokenSource $canceller
     * @param mixed[] $options
     * @param \Closure $onProgress
     * @param \Psr\Log\LoggerInterface|null $logger
     * @param \Amp\Promise $pause
     */
    private static function generateResponse($request, $multi, $id, &$info, &$headers, $canceller, &$options, $onProgress, &$handle, $logger, &$pause)
    {
        ...
        try {
            ...
            if (null === $response = yield from self::getPushedResponse($request, $multi, $info, $headers, $options, $logger)) {
                ($logger2 = $logger) ? $logger2->info(sprintf('Request: "%s %s"', $info['http_method'], $info['url'])) : null;
    ...
    /**
     * @param \Amp\Http\Client\Request $originRequest
     * @param \Symfony\Component\HttpClient\Internal\AmpClientState $multi
     * @param mixed[] $info
     * @param mixed[] $headers
     * @param \Amp\CancellationTokenSource $canceller
     ...
     * @param \Amp\Promise $pause
     */
    private static function followRedirects($originRequest, $multi, &$info, &$headers, $canceller, $options, $onProgress, &$handle, $logger, &$pause)
    {
        yield $pause;
        ...
        while (true) {
            ...
            ($logger2 = $logger) ? $logger2->info(sprintf('Redirecting: "%s %s"', $status, $info['url'])) : null;
     ...
     * @param mixed[] $headers
     */
    private static function addResponseHeaders($response, &$info, &$headers)
    {
     ...
     * @param \Psr\Log\LoggerInterface|null $logger
     */
    private static function getPushedResponse($request, $multi, &$info, &$headers, $options, $logger)
    {
        if ('' !== $options['body']) {
            return null;
        }
        ...
        foreach ($multi->pushedResponses[$authority] ?? [] as $i => [$pushedUrl, $pushDeferred, $pushedRequest, $pushedResponse, $parentOptions]) {
            ...
            foreach ($response->getHeaderArray('vary') as $vary) {
                foreach (preg_split('/\s*+,\s*+/', $vary) as $v) {
                    if ('*' === $v || ($pushedRequest->getHeaderArray($v) !== $request->getHeaderArray($v) && 'accept-encoding' !== strtolower($v))) {
                        ($logger2 = $logger) ? $logger2->debug(sprintf('Skipping pushed response: "%s"', $info['url'])) : null;
                        continue 3;
                    }
                }
            }
            ...
            ($logger2 = $logger) ? $logger2->debug(sprintf('Accepting pushed response: "%s %s"', $info['http_method'], $info['url'])) : null;
            self::addResponseHeaders($response, $info, $headers);
