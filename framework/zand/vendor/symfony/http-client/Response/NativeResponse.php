...
final class NativeResponse implements ResponseInterface, StreamableInterface
{
     ...
     */
    public function __construct($multi, $context, $url, $options, &$info, $resolver, $onProgress, $logger)
    {
        $this->multi = $multi;
    ...
    private function open()
    {
        ...
        set_error_handler(function ($type, $msg) use (&$url) {
            ...
            ($logger = $this->logger) ? $logger->info(sprintf('%s for "%s".', $msg, $url ?? $this->url)) : null;
        });
        ...
        try {
            ...
            while (true) {
                ...
                ($logger = $this->logger) ? $logger->info(sprintf('Redirecting: "%s %s"', $this->info['http_code'], $url ?? $this->url)) : null;
            }
        } catch (\Throwable $e) {
            $this->close();
