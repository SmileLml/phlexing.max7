...
final class NativeHttpClient implements HttpClientInterface, LoggerAwareInterface, ResetInterface
{
    ...
    /**
     * @var \Symfony\Component\HttpClient\Internal\NativeClientState
     */
    private $multi;
     ...
     */
    public function __construct($defaultOptions = [], $maxHostConnections = 6)
    {
    ...
    public function request($method, $url, $options = [])
    {
        ...
        ($logger = $this->logger) ? $logger->info(sprintf('Request: "%s %s"', $method, implode('', $url))) : null;
    ...
    private static function getBodyAsString($body)
    {
        if (\is_resource($body)) {
     ...
     * @param mixed[] $info
     */
    private static function parseHostPort($url, &$info)
    {
     ...
     * @param string $host
     * @param \Symfony\Component\HttpClient\Internal\NativeClientState $multi
     * @param mixed[] $info
     * @param \Closure|null $onProgress
     */
    private static function dnsResolve($host, $multi, &$info, $onProgress)
    {
     ...
     * @param \Closure|null $onProgress
     */
    private static function createRedirectResolver($options, $host, $port, $proxy, &$info, $onProgress)
    {
        $redirectHeaders = [];
     ...
     * @param bool $isSsl
     */
    private static function configureHeadersAndProxy($context, $host, $requestHeaders, $proxy, $isSsl)
    {
        if (null === $proxy) {
