...
final class CurlHttpClient implements HttpClientInterface, LoggerAwareInterface, ResetInterface
{
     ...
     * @var \Symfony\Component\HttpClient\Internal\CurlClientState
     */
    private $multi;
     ...
     */
    public function __construct($defaultOptions = [], $maxHostConnections = 6, $maxPendingPushes = 50)
    {
    ...
    public function request($method, $url, $options = [])
    {
        ...
        $curlopts = [
            ...
            \CURLOPT_SSLVERSION => (function () use ($options) {
                switch ($options['crypto_method']) {
        ...
        if ($pushedResponse = $this->multi->pushedResponses[$url] ?? null) {
            ...
            if (self::acceptPushForRequest($method, $options, $pushedResponse)) {
                ($logger = $this->logger) ? $logger->debug(sprintf('Accepting pushed response: "%s %s"', $method, $url)) : null;
                ...
                ($logger = $this->logger) ? $logger->debug(sprintf('Rejecting pushed response: "%s"', $url)) : null;
                $pushedResponse = null;
            }
        }
        ...
        if (!$pushedResponse) {
            $ch = curl_init();
            ($logger = $this->logger) ? $logger->info(sprintf('Request: "%s %s"', $method, $url)) : null;
     ...
     * @param string $method
     * @param mixed[] $options
     * @param \Symfony\Component\HttpClient\Internal\PushedResponse $pushedResponse
     */
    private static function acceptPushForRequest($method, $options, $pushedResponse)
    {
     ...
     * @param bool $eof
     */
    private static function readRequestBody($length, $body, &$buffer, &$eof)
    {
     ...
     * @param int $port
     */
    private static function createRedirectResolver($options, $host, $port)
    {
        $redirectHeaders = [];
     ...
     * @param int $opt
     */
    private function findConstantName($opt)
    {
     ...
     * @param mixed[] $options
     */
    private function validateExtraCurlOptions($options)
    {
        $curloptsToConfig = [
