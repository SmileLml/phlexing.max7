...
class sqlBuilder extends wg
{
     ...
     * @var mixed[]
     */
    protected static $tablesDesc = array();
    ...
    protected function buildWhereStep()
    {
        ...
        foreach($wheres as $index => $group)
        {
            ...
            foreach($items as $itemIndex => $item)
            {
                ...
                $groupItems[] = sqlBuilderWhereItem(set::first($itemIndex === 0), set::index("{$index}_{$itemIndex}"), set::tables($selectTableList), set::fields($fieldList), set::value($item), set::onChange('changeBuilderWhere(event)'), set::onAdd('addWhereItem(event)'), set::onRemove('removeWhereItem(event)'), set::tableError($builder->hasError('where', "{$index}_{$itemIndex}_0")), set::fieldError($builder->hasError('where', "{$index}_{$itemIndex}_1")), set::valueError($builder->hasError('where', "{$index}_{$itemIndex}_4")));
            }
