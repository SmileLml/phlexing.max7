...
class productsBox extends wg
{
    ...
    protected function initProductsBox()
    {
        global $lang, $app;
        ...
        (
            ...
            formGroup(on::change()->call('toggleNewProduct'), set::width('1/2'), setClass('linkProduct'), set::required($this->prop('required') || ($project && in_array($project->model, array('waterfall', 'waterfallplus')))), set::label($lang->project->manageProducts), set::labelFor('productBox'), $hasNewProduct ? set::checkbox(array('text' => $lang->project->addProduct, 'name' => 'addProduct', 'checked' => false)) : false, picker
            (
            ...
            formGroup(set::width('1/2'), set::label($typeLang), set::className($typeClass), inputGroup
            (
                set::id($typeIdAttr),
                picker
                (
     ...
     */
    protected function buildOnlyLinkPlans($productItems)
    {
        global $lang;
    ...
    /**
     * @param mixed[] $linkedProducts
     */
    protected function buildLinkedProducts($linkedProducts)
    {
        ...
        foreach(array_values($linkedProducts) as $i => $product)
        {
            ...
            $disabledProduct = !empty($project) && (in_array($product->id, $unmodifiableProducts) || $isStage || (data('disableModel') && $charterID));
